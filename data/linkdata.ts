const linkdata: { title: string; description: string; link: string; image: string }[] = [
	{ title: 'Porfolio', description: '직접 만든 포트폴리오 사이트', link: '/home', image: '/og_image_wt.png' },
	{ title: 'Github', description: '이것 저것 코드 조각', link: 'https://github.com/jeonghyoon', image: '/github.jpg' },
	{ title: 'Velog', description: '벨로그 기술 블로그', link: 'https://velog.io/@bbahna', image: '/velog.jpg' },
	{
		title: 'Naver Blog',
		description: '효동동 일상 블로그',
		link: 'https://m.blog.naver.com/moondomi_',
		// link: 'https://blog.naver.com/moondomi_',
		image: '/naver.png',
	},
	{
		title: 'Instagram',
		description: '소소한 일상 기록',
		link: 'https://www.instagram.com/jeong_hyoon',
		image: '/instar.jpg',
	},
];

export default linkdata;
