...
const PostList = ({ post, slug, isWide }: postProps) => {
	return (
		<Link
			href={`/posts/${slug}`}
			passHref
			className="group w-full relative p-2 my-2 duration-150 rounded-md hover:bg-gray-100 dark:hover:bg-zinc-800"
			onClick={() => firebaseLogging(`posts_${slug}_click`)}
		>
			{post.thumbnail && (
				<div
			...
			>
				<h2 className="text-[18px] font-bold theme-text-2 duration-200 theme-text-group-hover relative line-clamp-3">
					{post.title}
					{/* <NewMaker post={post} /> */}
				</h2>
