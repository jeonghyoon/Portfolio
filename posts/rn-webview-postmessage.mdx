...
> 이 프로젝트에서는 서버에서 refreshToken 쿠키를 `Max-Age=30일`로 설정했기에, iOS가 이를 **영구 쿠키**로 인식해 디스크에 보관합니다. 앱을 재 실행해도 쿠키가 그대로 남아있어서 별도의 Cookie 복원 로직이 필요 없어요. (관련 포스트: [React Native 앱-웹뷰 로그인 연동: iOS Cookie 유실 문제 해결하기](rn-webview-cookie-login.md))
